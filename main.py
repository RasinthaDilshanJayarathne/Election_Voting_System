...
@app.route('/vote/<string:nic>', methods=['GET', 'PUT', 'POST'])
def voted(nic):
    conn = db_connection()
    cursor = conn.cursor(dictionary=True)
    ...
    if request.method == 'GET':
        try:
            cursor.execute(
                "SELECT quota_name,nic,sum(voted)as count FROM vote_detail WHERE nic = %(nic)s", {'nic': nic})
            voted = cursor.fetchone()
            if voted:
                return jsonify(voted), 200
            else:
                return "Voted Details not found", 404
        except mysql.connector.Error as e:
            print(f"Error creating voted details: {e}")
            return "Failed to create voted details", 500

    if voted != 'null':
        if request.method == 'PUT':
            voted_data = request.get_json()
            try:
                sql = "UPDATE vote_detail SET voted = %(voted)s WHERE nic = %(nic)s"
                voted_data['nic'] = nic
                cursor.execute(sql, voted_data)
                conn.commit()
                return "Voted Details updated successfully", 200
            except mysql.connector.Error as e:
                print(f"Error creating voted details: {e}")
                return "Failed to create voted details", 500
    else:
        if request.method == 'POST':
            qnew_voted_data = request.get_json()
            try:
                sql = "INSERT INTO vote_detail (nic,quota_name,quota_reg_no,voted) VALUES (%(nic)s, %(quota_name)s, %(quota_reg_no)s, %(voted)s)"
                cursor.execute(sql, qnew_voted_data)
                conn.commit()
                return "Voted Details created successfully", 201
            except mysql.connector.Error as e:
                print(f"Error creating voted details: {e}")
                return "Failed to create voted details", 500
